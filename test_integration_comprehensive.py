...
class IntegrationTestSuite:
    ...
    def test_phi_compliance_integration(self):
        ...
        cleaned_text, detections = phi_detector.de_identify_text(test_letter, self.correlation_id)
        ...
        integration_results.append({
            ...
            'success': phi_detected and key_phi_removed and medical_content_preserved,
        ...
        with observability_context("integration_test", self.correlation_id) as (logger, metrics, monitor, corr_id):
            ...
            metrics_working = (
                'integration_tests' in snapshot['counters'] and
                'phi_processing_time' in snapshot['timers'] and
                snapshot['counters']['integration_tests'] > 0
            )
            ...
            integration_results.append({
                ...
                'metrics_collected': len(snapshot['counters']) + len(snapshot['timers'])
            })
    ...
    def test_end_to_end_analysis_workflow(self):
        ...
        try:
            ...
            start_time = time.time()
            cleaned_text, detections = phi_detector.de_identify_text(processed_text, self.correlation_id)
                ...
                return {
                    "analysis": {
                        ...
                        "service_connection": {
                            "score": 20,
                            "confidence": 80,
                            "findings": ["Clear connection to military service mentioned"],
                            "issues": [],
                        ...
                        "medical_rationale": {
                            "score": 18,
                            "confidence": 78,
                            "findings": ["Good medical rationale with symptom description"],
                            "issues": [],
            ...
            analysis_success = (
                not analysis_result.get('error', False) and
                'analysis' in analysis_result and
                analysis_result['analysis'].get('overall_score', 0) > 0
            )
            ...
            workflow_results.append({
                ...
                'overall_score': analysis_result.get('analysis', {}).get('overall_score', 0)
            })
            ...
            start_time = time.time()
            scoring_results = scorer.calculate_total_score(analysis_result.get('analysis', {}))
            scoring_time = (time.time() - start_time) * 1000
            
            scoring_success = (
                'overall_score' in scoring_results and
            ...
            analysis_id = database.save_analysis(
                test_letter, analysis_result.get('analysis', {}),
                scoring_results, analysis_result.get('workflow_recommendation', {}),
        ...
        total_processing_time = sum(r.get('processing_time', 0) for r in workflow_results)
    ...
    def test_production_ui_integration(self):
        ...
        ui_tests.append({
            ...
            'status_components': len([c for c in mock_st.components_created if 'Status' in c[1]])
        })
    ...
    def test_production_configuration_validation(self):
        ...
        for var in required_env_vars:
            ...
            env_var_status[var] = {
                ...
                'masked_value': (value[:8] + '...' + value[-4:]) if value and len(value) > 12 else 'NOT_SET'
            }
        ...
        try:
            phi_detector = create_phi_detector()
            phi_test_result = phi_detector.detect_phi("Test John Smith", self.correlation_id)
    ...
    def test_performance_under_load(self):
        ...
        with observability_context("load_test", self.correlation_id) as (logger, metrics, monitor, corr_id):
            for i in range(50):
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'overall_status': 'PASS' if pass_rate >= 80 else 'FAIL',  # Lower threshold due to potential component unavailability
