...
class TextProcessor:
    ...
    def clean_text(self, raw_text: str) -> str:
        ...
        text = re.sub(r'([a-z])([A-Z])', r'\1 \2', text)     # Fix merged words (basic case)
    ...
    def validate_input(self, text: str) -> Tuple[bool, str]:
        ...
        if len(text) < self.min_length:
            return False, f"Text is too short. Please enter at least {self.min_length} characters (currently {len(text)})"
        
        if len(text) > self.max_length:
            return False, f"Text is too long. Maximum {self.max_length} characters allowed (currently {len(text)})"
        
        # Check for meaningful content
        if not self._has_meaningful_content(text):
            return False, "Text appears to lack meaningful medical or legal content for analysis"
        
        # Check for potential nexus letter indicators
        if not self._appears_to_be_nexus_letter(text):
            return False, "Text does not appear to be a nexus letter. Please ensure it contains medical opinions about service connection."
    ...
    def _appears_to_be_nexus_letter(self, text: str) -> bool:
        ...
        medical_indicators = [
            'medical opinion', 'professional opinion', 'clinical opinion',
            'medical assessment', 'diagnosis', 'condition', 'treatment',
            'medical history', 'symptoms', 'medical records'
        ]
        
        # Service connection indicators
        service_indicators = [
            'service connect', 'military service', 'veteran', 'va ',
            'disability', 'service-related', 'service related',
            'likely as not', 'probability', 'nexus', 'secondary to'
        ]
        
        # Probability language indicators
        probability_indicators = [
            'likely as not', 'probability', 'probable', 'possible',
            'medical certainty', 'reasonable', 'opinion',
            'at least as likely', 'more likely than not'
        ]
        ...
        has_probability = any(indicator in text_lower for indicator in probability_indicators)
    ...
    def extract_letter_components(self, text: str) -> Dict[str, Optional[str]]:
        ...
        for i, line in enumerate(lines[:10]):  # Check first 10 lines
            if line.strip() and not line.strip().lower().startswith(('dear', 'to whom')):
        ...
        for line in lines:
            ...
            if in_body and (
                line_clean.lower().startswith(('sincerely', 'respectfully', 'regards')) or
                re.search(r'(M\.?D\.?|Ph\.?D\.?|D\.?O\.?)', line_clean)
            ):
                break
