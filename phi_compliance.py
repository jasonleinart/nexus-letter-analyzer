...
class PHIDetectionEngine:
    ...
    def _initialize_false_positives(self) -> Dict[PHICategory, set]:
        ...
        return {
            PHICategory.NAME: {
                'nexus letter', 'medical center', 'veterans affairs', 'department of',
                'united states', 'social security', 'disability law', 'law group',
                'claims examiner', 'rating decision', 'service connection',
                'medical opinion', 'nexus opinion', 'dear sir', 'to whom',
                'thank you', 'best regards', 'medical associates', 'health care',
                'primary care', 'mental health', 'back pain', 'knee pain',
                'shoulder pain', 'ptsd condition', 'hearing loss', 'tbi symptoms'
            }
        }
    ...
    def _initialize_patterns(self) -> Dict[PHICategory, List[re.Pattern]]:
        ...
        patterns[PHICategory.NAME] = [
            ...
            re.compile(r'(?:patient|veteran|individual|person|claimant|mr\.?|mrs\.?|ms\.?|miss)\s+([A-Z][a-z]+\s+[A-Z]\.?\s*[A-Z][a-z]+)', re.IGNORECASE),
            ...
            re.compile(r'(?:Dr\.?|Doctor)\s+([A-Z][a-z]+\s+[A-Z]\.?\s*[A-Z][a-z]+)(?:,\s*M\.?D\.?)?', re.IGNORECASE),
            re.compile(r'Sincerely,?\s*([A-Z][a-z]+\s+[A-Z]\.?\s*[A-Z][a-z]+)', re.IGNORECASE),
            ...
            re.compile(r'(?:examined|treated|seen|evaluated)\s+([A-Z][a-z]+\s+[A-Z][a-z]+)', re.IGNORECASE),
        ]
        ...
        patterns[PHICategory.DATE_OF_BIRTH] = [
            re.compile(r'(?:born|birth|dob|date of birth)[\s:]*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})', re.IGNORECASE),
            re.compile(r'(?:born|birth)[\s:]*(?:on\s+)?([A-Z][a-z]+\s+\d{1,2},?\s+\d{4})', re.IGNORECASE),
            re.compile(r'\b(?:DOB|D\.O\.B\.?)[\s:]*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})', re.IGNORECASE),
            # Specific birth contexts
            re.compile(r'\b(\d{1,2}[/-]\d{1,2}[/-]\d{4})\b(?=.*born|.*birth)', re.IGNORECASE),
        ]
        
        # Address patterns (improved specificity)
        patterns[PHICategory.ADDRESS] = [
            # Street addresses with numbers
            re.compile(r'\b\d+\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\s+(?:St|Street|Ave|Avenue|Rd|Road|Blvd|Boulevard|Dr|Drive|Ln|Lane|Ct|Court|Pl|Place)\.?\b'),
            # PO Boxes
        ...
        patterns[PHICategory.MEDICAL_RECORD_NUMBER] = [
            re.compile(r'\b(?:MRN|medical record|patient id|chart number)[\s:]*([A-Z0-9-]+)', re.IGNORECASE),
    ...
    def detect_phi(self, text: str, correlation_id: Optional[str] = None) -> List[PHIDetection]:
        """
    ...
    def _is_false_positive(self, category: PHICategory, text: str) -> bool:
        ...
        if category == PHICategory.NAME:
            ...
            medical_terms = [
                'back pain', 'knee joint', 'left arm', 'right leg', 'chest pain',
                'experiences back', 'and hearing', 'hearing loss', 'experiences pain',
                'and depression', 'and anxiety', 'back injury', 'knee injury',
                'shoulder pain', 'neck pain', 'headache', 'and fatigue'
            ]
            ...
            medical_legal_terms = [
                'nexus opinion', 'medical nexus', 'nexus letter', 'nexus evaluation',
                'medical opinion', 'legal opinion', 'disability opinion',
                'va nexus', 'nexus assessment', 'nexus examination'
            ]
            ...
            verb_patterns = ['experiences', 'has', 'reports', 'exhibits', 'shows', 'presents']
    ...
    def _remove_overlapping_detections(self, detections: List[PHIDetection]) -> List[PHIDetection]:
        ...
        for detection in detections:
            ...
            for accepted in filtered:
                if (detection.start_position < accepted.end_position and 
                    detection.end_position > accepted.start_position):
                    overlaps = True
                    break
    ...
    def de_identify_text(self, text: str, correlation_id: Optional[str] = None) -> Tuple[str, List[PHIDetection]]:
        """
    ...
    def _calculate_confidence(self, category: PHICategory, text: str, full_text: str) -> float:
        ...
        if category == PHICategory.NAME:
            ...
            context_indicators = {
                'patient': 1.0, 'veteran': 1.0, 'mr.': 0.9, 'mrs.': 0.9, 'ms.': 0.9,
                'examined': 0.95, 'treated': 0.95, 're:': 0.9, 'sincerely': 0.85,
                'dr.': 0.8  # Could be doctor name or patient name
            }
            
            text_lower = full_text.lower()
            max_context_confidence = 0.7  # Base confidence for names without strong context
            ...
            if any(term in full_text.lower() for term in ['dob', 'date of birth', 'born']):
                return 0.95
            return 0.8
    ...
    def _log_phi_detection(self, correlation_id: str, detections: List[PHIDetection], original_text: str):
...
class SecureAuditLogger:
    ...
    def log_analysis_start(self, correlation_id: str, text_length: int, user_id: Optional[str] = None):
    ...
    def log_phi_processing(self, correlation_id: str, phi_detected: bool, 
                          categories: List[PHICategory], processing_time_ms: int):
    ...
    def log_ai_request(self, correlation_id: str, model: str, tokens_sent: int, 
                      tokens_received: int, processing_time_ms: int):
    ...
    def log_analysis_complete(self, correlation_id: str, overall_score: int, 
                            total_processing_time_ms: int, success: bool):
...
class DataRetentionManager:
    ...
    def should_retain_data(self, created_date: datetime, retention_days: Optional[int] = None) -> bool:
        """
    ...
    def get_eligible_for_deletion(self, database_connection) -> List[int]:
        ...
        """
        cutoff_date = datetime.utcnow().date() - timedelta(days=self.default_retention_days)
    ...
    def secure_delete_analysis(self, database_connection, analysis_id: int, correlation_id: str) -> bool:
        """
        ...
        try:
            ...
            cursor.execute("DELETE FROM component_scores WHERE analysis_id = ?", (analysis_id,))
            ...
            self.logger.error(f"Secure deletion failed for analysis {analysis_id}: {str(e)}")
            return False
...
def phi_compliant_processing(correlation_id: Optional[str] = None):
    ...
    try:
        ...
        audit_logger.logger.info(f"PHI_PROCESSING_SESSION_END: {correlation_id} - {processing_time:.0f}ms")
...
if __name__ == "__main__":
    ...
    with phi_compliant_processing() as (corr_id, detector, audit):
        ...
        print("\n=== DETECTIONS ===")
        for detection in detections:
            print(f"- {detection.category.value}: '{detection.original_text}' -> '{detection.redacted_text}' (confidence: {detection.confidence:.2f})")
        ...
        assert "nexus letter" in cleaned_text.lower(), "Medical terminology should be preserved"
        assert "service connection" in cleaned_text.lower(), "Legal terminology should be preserved"
