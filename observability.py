...
class PerformanceMonitor:
    ...
    def start_request_tracking(
        self, request_id: str, correlation_id: Optional[str] = None
    ):
        """Start tracking a request."""
        with self._lock:
            self._active_requests[request_id] = time.time()
            self.metrics.increment_counter(
                "requests_started", correlation_id=correlation_id
            )
        ...
        with self._lock:
            ...
            if start_time:
                ...
                self.metrics.record_timer(
                    "request_duration_ms", duration_ms, correlation_id=correlation_id
                )

                # Record success/failure
                if success:
                    self.metrics.increment_counter(
                        "requests_successful", correlation_id=correlation_id
                    )
                else:
                    self.metrics.increment_counter(
                        "requests_failed", correlation_id=correlation_id
                    )
    ...
    def record_phi_detection(self, count: int, correlation_id: Optional[str] = None):
        ...
        self.metrics.increment_counter(
            "phi_detections_total", count, correlation_id=correlation_id
        )
        self.metrics.set_gauge(
            "phi_detections_last", count, correlation_id=correlation_id
        )

    def record_fallback_response(self, correlation_id: Optional[str] = None):
        """Record fallback response usage."""
        self.metrics.increment_counter(
            "fallback_responses", correlation_id=correlation_id
        )

    def record_circuit_breaker_event(
        self, breaker_name: str, state: str, correlation_id: Optional[str] = None
    ):
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        error_rate_threshold = (
            0.25  # > 25% error rate = unhealthy (high_error_rate: 30% error)
        )
...
def observability_context(
    ...
    try:
        logger.info(
            f"Starting {component_name} processing",
        ...
        # Failure
        monitor.end_request_tracking(
            correlation_id, success=False, correlation_id=correlation_id
        )
        duration_ms = (time.time() - request_start) * 1000
        logger.error(
            f"Failed {component_name} processing", error=e, duration_ms=duration_ms
        )
        raise
...
def with_observability(component_name: str):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            with observability_context(component_name, correlation_id) as (
                logger,
                metrics,
                monitor,
                corr_id,
...
def create_structured_logger(
    component_name: str, log_file: Optional[str] = None
) -> StructuredLogger:
...
if __name__ == "__main__":
    ...
    def test_observability():
        ...
        with observability_context("test_component") as (
            logger,
            metrics,
            monitor,
            corr_id,
