...
class AnalysisDatabase:
    ...
    def save_analysis(self, letter_text: str, ai_analysis: Dict, 
                     scoring_results: Dict, recommendations: Dict,
        ...
        try:
            ...
            workflow_decision = workflow_rec.decision if hasattr(workflow_rec, 'decision') else 'unknown'
            
            # Extract component scores
            medical_opinion_score = self._extract_component_score(scoring_results, 'medical_opinion')
            service_connection_score = self._extract_component_score(scoring_results, 'service_connection')
            medical_rationale_score = self._extract_component_score(scoring_results, 'medical_rationale')
            professional_format_score = self._extract_component_score(scoring_results, 'professional_format')
            ...
            scoring_details_json = json.dumps(self._serialize_scoring_results(scoring_results))
            recommendations_json = json.dumps(self._serialize_recommendations(recommendations))
            ...
            with self._get_connection() as conn:
                ...
                existing = cursor.execute(
                    "SELECT id FROM analyses WHERE letter_hash = ?", 
                    (letter_hash,)
                ).fetchone()
                
                if existing:
                    logger.info(f"Analysis already exists for this letter (ID: {existing['id']})")
                    return existing['id']
                ...
                """, (
                    letter_hash, letter_preview, overall_score,
                    workflow_rec.message if hasattr(workflow_rec, 'message') else '',
                    workflow_decision, medical_opinion_score, service_connection_score,
                    medical_rationale_score, professional_format_score,
                    ai_response_json, scoring_details_json, recommendations_json,
                    processing_time, 
                    ...
                    critical_issues, len(improvements)
                ))
    ...
    def get_analysis(self, analysis_id: int) -> Optional[Dict]:
        ...
        try:
            with self._get_connection() as conn:
                ...
                result['scoring_details'] = json.loads(result.pop('scoring_details_json'))
                result['recommendations'] = json.loads(result.pop('recommendations_json'))
                
                # Get component scores
                result['component_scores'] = self._get_component_scores(conn, analysis_id)
        ...
        try:
            with self._get_connection() as conn:
                ...
                return {
                    ...
                    'component_performance': dict(component_performance) if component_performance else {}
                }
    ...
    def _save_component_scores(self, conn: sqlite3.Connection, 
                              analysis_id: int, scoring_results: Dict):
            ...
            if breakdown_key in scoring_results:
                ...
                if hasattr(breakdown, 'score'):
                    cursor.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?)
                    """, (
                        analysis_id, component, breakdown.score, breakdown.max_score,
                        json.dumps(breakdown.criteria) if hasattr(breakdown, 'criteria') else '{}'
                    ))
    
    def _save_improvements(self, conn: sqlite3.Connection, 
                          analysis_id: int, improvements: List):
        """Save improvement suggestions."""
        cursor = conn.cursor()
    ...
    def _update_analytics_summary(self, conn: sqlite3.Connection):
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (
            today, metrics['total'], metrics['avg_score'], 
            metrics['auto_approve'], metrics['attorney_review'],
            metrics['revision_required'], metrics['avg_time']
        ))
    
    def _get_component_scores(self, conn: sqlite3.Connection, 
                             analysis_id: int) -> List[Dict]:
    ...
    def _get_improvements(self, conn: sqlite3.Connection, 
                         analysis_id: int) -> List[Dict]:
