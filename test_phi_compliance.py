...
def test_phi_detection_patterns():
    ...
    try:
        ...
        phi_keywords = ['phi', 'deidentify', 'redact', 'patient', 'ssn', 'social security']
        ...
        if phi_patterns_found:
            log_test_result("PHI Detection Code Present", "⚠️  PARTIAL",
                           "Found PHI-related keywords in code")
        else:
            log_test_result("PHI Detection Code Present", "❌ FAIL",
    ...
    log_test_result("Basic PHI Pattern Detection (Manual)", "ℹ️  INFO",
        ...
        with sqlite3.connect('nexus_analyses.db') as conn:
            ...
            for table_name in tables:
                ...
                for column in columns:
                    ...
                    if any(phi_indicator in column_name for phi_indicator in 
                          ['patient', 'name', 'ssn', 'phone', 'address', 'dob']):
                        phi_risk_columns.append(f"{table_name}.{column[1]}")
            
            if phi_risk_columns:
                log_test_result("Database PHI Risk Columns", "⚠️  WARNING",
                               f"Found potential PHI columns: {', '.join(phi_risk_columns)}")
            else:
                log_test_result("Database PHI Risk Columns", "✅ PASS",
            ...
            for analysis in sample_analyses:
                ...
                if hasattr(analysis, 'letter_text') and analysis.letter_text:
                    ...
                    if any(indicator in text for indicator in 
                          ['patient:', 'ssn:', 'date of birth', 'phone:', 'mr-']):
                        phi_risk_data.append(analysis.id)
        
        if phi_risk_data:
            log_test_result("Database PHI Content Risk", "❌ CRITICAL",
                           f"Found {len(phi_risk_data)} analyses with potential PHI")
        else:
            log_test_result("Database PHI Content Risk", "✅ PASS",
...
def test_logging_phi_exposure():
    ...
    log_test_result("Logging Handlers Found", "ℹ️  INFO",
    ...
    try:
        ...
        log_statements_with_data = content.count('logger.info(') + content.count('logger.debug(')
        
        log_test_result("Logging Statements Found", "ℹ️  INFO",
        ...
        if 'letter_text' in content and 'logger' in content:
            log_test_result("Potential PHI in Logs", "⚠️  WARNING",
                           "Code may log letter_text which could contain PHI")
        else:
            log_test_result("Potential PHI in Logs", "✅ PASS", 
...
def test_audit_trail_capabilities():
    ...
    try:
        with sqlite3.connect('nexus_analyses.db') as conn:
            ...
            audit_tables = [t for t in tables if 'audit' in t.lower() or 'log' in t.lower()]
            
            if audit_tables:
                log_test_result("Audit Tables Present", "✅ PASS",
                               f"Found audit tables: {', '.join(audit_tables)}")
            else:
                log_test_result("Audit Tables Present", "❌ FAIL",
    ...
    try:
        ...
        with sqlite3.connect('nexus_analyses.db') as conn:
            ...
            correlation_columns = [col for col in columns if 
        ...
        if correlation_features:
            log_test_result("Request Correlation Support", "✅ PASS",
                           f"Found correlation columns: {', '.join(correlation_features)}")
        else:
            log_test_result("Request Correlation Support", "❌ FAIL",
...
def test_configuration_security():
    ...
    try:
        ...
        if hardcoded_keys:
            log_test_result("Hardcoded Credentials", "❌ CRITICAL",
                           f"Found potential hardcoded keys: {', '.join(hardcoded_keys)}")
        else:
            log_test_result("Hardcoded Credentials", "✅ PASS",
    ...
    try:
        ...
        if env_vars_used:
            log_test_result("Environment Variable Security", "✅ PASS",
                           f"Using environment variables: {', '.join(env_vars_used)}")
        else:
            log_test_result("Environment Variable Security", "⚠️  WARNING",
...
def test_data_retention_policies():
            ...
            if os.path.exists(file_path):
                with open(file_path, 'r') as f:
                    ...
                    if any(term in content.lower() for term in 
                          ['retention', 'cleanup', 'delete', 'expire']):
                        retention_config_found = True
                        break
        
        if retention_config_found:
            log_test_result("Data Retention Configuration", "✅ PASS",
                           "Found retention-related code")
        else:
            log_test_result("Data Retention Configuration", "❌ FAIL",
    ...
    try:
        db = create_database()
        has_delete_methods = hasattr(db, 'delete_analysis') or hasattr(db, 'cleanup_old_data')
        
        if has_delete_methods:
            log_test_result("Secure Deletion Methods", "✅ PASS",
                           "Database has deletion methods")
        else:
            log_test_result("Secure Deletion Methods", "❌ FAIL",
                           "No secure deletion methods found")
