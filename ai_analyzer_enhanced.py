...
from error_handling import (
    ...
    CircuitBreaker, create_circuit_breaker, ErrorClassifier
)
from observability import (
    observability_context, create_structured_logger, 
    create_performance_monitor, StructuredLogger
)
...
class ProductionNexusLetterAnalyzer:
    ...
    def __init__(self, api_key: Optional[str] = None, enable_phi_protection: bool = True):
        """
    ...
    def _initialize_production_components(self):
        ...
        self.performance_monitor = create_performance_monitor("ai_analyzer", "ai_analyzer_metrics.log")
    ...
    def test_connection(self, correlation_id: Optional[str] = None) -> Tuple[bool, str]:
        ...
        with observability_context("connection_test", correlation_id) as (logger, metrics, monitor, corr_id):
            try:
                ...
                logger.error(f"API connection test failed: {error_category.value}", error=e)
                monitor.record_circuit_breaker_event("openai_api", "failure", corr_id)
                
                return False, f"Connection failed: {str(e)}"
    
    def analyze_letter(self, letter_text: str, correlation_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        with observability_context("letter_analysis", correlation_id) as (logger, metrics, monitor, corr_id):
            ...
            try:
                ...
                if phi_detections:
                    logger.warning(f"PHI detected and redacted: {len(phi_detections)} instances")
                ...
                logger.info("Letter analysis completed successfully", duration_ms=processing_time)
                return ai_result
                ...
                logger.error(f"Letter analysis failed: {error_category.value}", 
                           error=e, duration_ms=processing_time)
    ...
    def _protect_phi(self, letter_text: str, correlation_id: str) -> Tuple[str, list]:
        ...
        with phi_compliant_processing(correlation_id) as (corr_id, phi_detector, audit_logger):
            ...
            cleaned_text, detections = phi_detector.de_identify_text(letter_text, corr_id)
            
            # Audit PHI processing
            categories = list(set(d.category for d in detections))
            audit_logger.log_phi_processing(
                corr_id, len(detections) > 0, categories, 0  # Processing time tracked elsewhere
            )
    ...
    def _perform_ai_analysis(self, letter_text: str, correlation_id: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.enable_phi_protection:
                error_category = ErrorClassifier.classify_error(e)
                self.audit_logger.log_error(correlation_id, error_category.value, str(e))
...
Provide ONLY the JSON response, no additional text or formatting.
    ...
    def _parse_response(self, response_text: str, correlation_id: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.info("AI response parsed successfully", metadata={'correlation_id': correlation_id})
            ...
            self.logger.error(f"JSON parsing failed", error=e, metadata={'correlation_id': correlation_id})
            return self._create_fallback_response(response_text, f"JSON error: {str(e)}", correlation_id)
        except Exception as e:
            self.logger.error(f"Response parsing failed", error=e, metadata={'correlation_id': correlation_id})
            return self._create_fallback_response(response_text, str(e), correlation_id)
    
    def _create_fallback_response(self, raw_response: str, error_details: str, correlation_id: str) -> Dict[str, Any]:
        """
        ...
        """
        self.logger.warning(f"Creating fallback response due to: {error_details}", 
    ...
    def _enrich_with_production_metadata(self, ai_result: Dict[str, Any], 
                                       phi_detections: list, processing_time: float,
...
def create_analyzer(enable_phi_protection: bool = True) -> ProductionNexusLetterAnalyzer:
