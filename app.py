...
def display_sidebar_info():
    ...
    with st.sidebar:
        ...
        **Requirements:**
        ...
        st.markdown("---")
        st.markdown("**Professional AI-Powered Legal Analysis Tool**")
        st.markdown("*Advanced GPT-4 Integration for Legal Workflows*")
...
def display_component_analysis(scoring_results: Dict[str, Any]):
    ...
    components = [
        ...
        (
            "Service Connection",
            scoring_results.get("service_connection_breakdown"),
            "🎖️",
        ),
        ("Medical Rationale", scoring_results.get("medical_rationale_breakdown"), "🧠"),
        (
            "Professional Format",
            scoring_results.get("professional_format_breakdown"),
            "📝",
        ),
    ]
                ...
                with st.expander(f"View {name} Details"):
                    if hasattr(breakdown, "criteria") and breakdown.criteria:
                        ...
                        for criterion, points in breakdown.criteria.items():
                            st.markdown(
                                f"• **{criterion.replace('_', ' ').title()}:** {points} points"
                            )
...
def display_findings_and_recommendations(
    ...
    with col1:
        ...
        if analysis.get("primary_condition") or analysis.get("probability_language"):
            ...
            if analysis.get("probability_language"):
                st.info(
                    f"**Probability Language:** \"{analysis['probability_language']}\""
                )
    ...
    with col2:
        ...
        if improvements:
            ...
            st.info(
                "No specific improvements identified - letter meets basic standards."
            )
...
def display_performance_metrics(
    processing_time: float, analysis: Dict[str, Any], recommendations: Dict[str, Any]
):
...
def format_enhanced_results_for_export(
    ...
    if improvements:
        ...
        for i, imp in enumerate(improvements[:10], 1):
            if hasattr(imp, "component"):
                lines.append(
                    f"{i}. {imp.component.replace('_', ' ').title()} ({imp.impact})"
                )
    ...
    if client_summary:
        lines.extend(["", "CLIENT SUMMARY:", "-" * 40, client_summary])
    ...
    lines.extend(
        [
            ...
            f"© Professional Legal AI Analysis Tool",
        ]
    )
