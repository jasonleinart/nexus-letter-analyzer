...
class PHIComplianceTestSuite:
    ...
    def test_hipaa_safe_harbor_categories(self):
        ...
        test_cases = {
            ...
            "phone_numbers": ["(555) 123-4567", "555-987-6543", "555.123.4567"],
            "emails": ["john.smith@email.com", "patient.care@hospital.org"],
            "medical_record_numbers": [
                "MRN: ABC123456",
                "Medical Record Number: MR-98765",
                "Patient ID: PAT789012",
            ],
            "account_numbers": ["Account: 987654321", "Acct# 456789123"],
            "license_numbers": ["License: MD123456", "Medical License: LIC789012"],
            "urls": [
        ...
        for category, test_texts in test_cases.items():
            ...
            for test_text in test_texts:
                detections = self.phi_detector.detect_phi(
                    test_text, self.correlation_id
                )
                category_tests += 1
        ...
        overall_success = sum(r["detections"] for r in category_results.values()) >= (
            sum(r["tests"] for r in category_results.values()) * 0.9
        )
        ...
        self.log_test_result(
            ...
            "PASS" if overall_success else "FAIL",
            {
                ...
                "overall_detection_rate": (
                    total_detections
                    / sum(r["tests"] for r in category_results.values())
                )
                * 100,
        ...
        for test_text in false_positive_tests:
            ...
            if name_detections:
                ...
                for detection in name_detections:
                    print(
                        f"   False positive: '{detection.original_text}' in '{test_text}'"
                    )
    ...
    def test_comprehensive_nexus_letter_phi_detection(self):
        ...
        category_coverage = len(
            detected_categories.intersection(expected_categories)
        ...
        success = (
            len(detections) >= 8  # Should detect at least 8 PHI elements
            and category_coverage >= 0.7  # Should cover 70%+ of expected categories
            and phi_properly_redacted
    ...
    def test_audit_logging_compliance(self):
        ...
        with phi_compliant_processing(self.correlation_id) as (
            corr_id,
            detector,
            audit,
        ):
        ...
        success = (
            ...
            and len(detections) > 0  # Should detect PHI in test text
        )
    ...
    def test_performance_benchmarks(self):
        ...
        test_cases = [
            ...
            (
                "Medium nexus letter content " * 20 + " John A. Smith SSN: 123-45-6789",
                500,
            ),
            (
                "Long comprehensive nexus letter " * 100
                + " with PHI elements scattered throughout John Smith, Dr. Wilson, 123-45-6789, john@email.com",
                2000,
            ),
        ]
            ...
            performance_results.append(
                {
                    ...
                    "detections_count": len(detections),
                    "performance_ratio": processing_time
                    / len(test_text),  # ms per character
                }
            )
        ...
        avg_processing_time = sum(
            r["processing_time_ms"] for r in performance_results
    ...
    def test_confidence_scoring_accuracy(self):
        ...
        accuracy_rate = sum(
            1 for r in confidence_results if r["meets_expectation"]
