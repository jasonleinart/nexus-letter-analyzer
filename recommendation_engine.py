...
class RecommendationEngine:
    ...
    def generate_recommendations(self, overall_score: int, component_scores: Dict, 
        ...
        try:
            ...
            return {
                ...
                'critical_issues': len([i for i in improvements if i.impact == 'critical'])
            }
    ...
    def generate_workflow_recommendation(self, overall_score: int) -> WorkflowRecommendation:
        """
    ...
    def generate_improvement_suggestions(self, component_scores: Dict, 
                                       ai_analysis: Dict) -> List[ImprovementSuggestion]:
        """
        ...
        for component in components:
            if component in ai_analysis:
                ...
                suggestions.extend(self._map_issues_to_suggestions(component, issues, comp_score))
                
                # Check for low scores even without specific issues
                if hasattr(comp_score, 'score') and comp_score.score < 15:
                    suggestions.extend(self._generate_low_score_suggestions(component, comp_analysis))
        
        return suggestions
    
    def _map_issues_to_suggestions(self, component: str, issues: List[str], 
                                  score_breakdown: any) -> List[ImprovementSuggestion]:
    ...
    def _issue_matches_template(self, issue_text: str, template_key: str) -> bool:
        ...
        matching_patterns = {
            'missing_probability': ['no probability', 'missing probability', 'lacks probability'],
            'weak_probability': ['weak', 'speculative', 'uncertain', 'may be', 'could be'],
            'no_clear_opinion': ['unclear', 'equivocal', 'ambiguous'],
            'no_specific_events': ['no specific', 'lacks detail', 'vague'],
            'unclear_linkage': ['connection unclear', 'linkage not', 'relationship unclear'],
            ...
            'missing_credentials': ['credentials not', 'no credentials', 'physician info'],
    ...
    def _generate_low_score_suggestions(self, component: str, 
                                      comp_analysis: Dict) -> List[ImprovementSuggestion]:
        ...
        suggestion = ImprovementSuggestion(
            ...
            impact='high' if component in ['medical_opinion', 'service_connection'] else 'medium'
        )
        suggestions.append(suggestion)
        
        return suggestions
    
    def prioritize_improvements(self, improvements: List[ImprovementSuggestion]) -> List[ImprovementSuggestion]:
        """
    ...
    def generate_client_summary(self, overall_score: int, 
                              workflow_rec: WorkflowRecommendation,
                              improvements: List[ImprovementSuggestion]) -> str:
        """
    ...
    def generate_attorney_notes(self, component_scores: Dict,
                              improvements: List[ImprovementSuggestion]) -> str:
        """
