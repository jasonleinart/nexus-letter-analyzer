...
def test_milestone2_integration():
    ...
    try:
        ...
        print(
            f"   ✓ Text processed: {stats['word_count']} words, {stats['sentence_count']} sentences\n"
        )
        ...
        print(
            f"   - Probability Language: {analysis.get('probability_language', 'Not found')}\n"
        )
        ...
        for comp in components:
            ...
            if breakdown and hasattr(breakdown, "score"):
                print(
                    f"   - {comp.replace('_', ' ').title()}: {breakdown.score}/{breakdown.max_score}"
                )
        print()
        ...
        recommendations = rec_engine.generate_recommendations(
            scoring_results["overall_score"], scoring_results, analysis
        )
        ...
        analysis_id = database.save_analysis(
            test_letter, analysis, scoring_results, recommendations, processing_time
        )
        ...
        print(
            f"✓ Workflow recommendation: {workflow_rec.decision.replace('_', ' ').title()}"
        )
