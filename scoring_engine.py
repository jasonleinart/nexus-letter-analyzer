...
class VAComplianceScorer:
    ...
    def calculate_total_score(self, ai_analysis: Dict) -> Dict:
        ...
        try:
            ...
            medical_opinion_result = self.calculate_medical_opinion_score(ai_analysis.get('medical_opinion', {}))
            service_connection_result = self.calculate_service_connection_score(ai_analysis.get('service_connection', {}))
            medical_rationale_result = self.calculate_medical_rationale_score(ai_analysis.get('medical_rationale', {}))
            professional_format_result = self.calculate_professional_format_score(ai_analysis.get('professional_format', {}))
    ...
    def calculate_medical_opinion_score(self, component_analysis: Dict) -> ScoreBreakdown:
        """
        ...
        elif len(issues) > 0 and any('equivocal' in issue.lower() or 'unclear' in issue.lower() for issue in issues):
        ...
        return ScoreBreakdown(
            ...
            rationale=self._generate_component_rationale('medical_opinion', criteria_scores, findings, issues)
        )
    
    def calculate_service_connection_score(self, component_analysis: Dict) -> ScoreBreakdown:
        """
        ...
        if any('service' in finding.lower() and 'condition' in finding.lower() for finding in findings):
        ...
        elif any('during' in finding.lower() or 'after' in finding.lower() for finding in findings):
        ...
        return ScoreBreakdown(
            ...
            max_score=25,
            criteria=criteria_scores,
            rationale=self._generate_component_rationale('service_connection', criteria_scores, findings, issues)
        )
    
    def calculate_medical_rationale_score(self, component_analysis: Dict) -> ScoreBreakdown:
        """
        ...
        elif any('medical' in finding.lower() or 'clinical' in finding.lower() for finding in findings):
        ...
        elif any('research' in finding.lower() or 'study' in finding.lower() for finding in findings):
        ...
        return ScoreBreakdown(
            ...
            max_score=25,
            criteria=criteria_scores,
            rationale=self._generate_component_rationale('medical_rationale', criteria_scores, findings, issues)
        )
    
    def calculate_professional_format_score(self, component_analysis: Dict) -> ScoreBreakdown:
        """
        ...
        elif any('dr' in finding.lower() or 'physician' in finding.lower() for finding in findings):
        ...
        elif any('address' in finding.lower() or 'phone' in finding.lower() for finding in findings):
            criteria_scores['contact_info'] = 3
        
        # Proper structure (6 points max)
        if len(findings) > 2 and not any('poor structure' in issue.lower() for issue in issues):
        ...
        return ScoreBreakdown(
            ...
            max_score=25,
            criteria=criteria_scores,
            rationale=self._generate_component_rationale('professional_format', criteria_scores, findings, issues)
        )
    ...
    def _has_weak_probability_language(self, findings: List[str], issues: List[str]) -> bool:
    ...
    def _has_definitive_opinion(self, findings: List[str], issues: List[str]) -> bool:
        ...
        has_positive = any(indicator in findings_text for indicator in positive_indicators)
        has_negative = any(indicator in issues_text for indicator in negative_indicators)
        
        return has_positive and not has_negative
    
    def _has_specific_service_events(self, findings: List[str]) -> bool:
        """Check for specific service events or exposures."""
        event_indicators = ['deployment', 'combat', 'exposure', 'incident', 'injury', 'event']
        findings_text = ' '.join(findings).lower()
    ...
    def _has_temporal_relationship(self, findings: List[str]) -> bool:
        ...
        temporal_indicators = ['during service', 'in service', 'while serving', 'onset', 'began', 'started']
        findings_text = ' '.join(findings).lower()
    ...
    def _has_clinical_explanation(self, findings: List[str]) -> bool:
        ...
        clinical_indicators = ['pathophysiology', 'mechanism', 'etiology', 'clinical', 'medical basis']
        findings_text = ' '.join(findings).lower()
    ...
    def _has_literature_references(self, findings: List[str]) -> bool:
        ...
        literature_indicators = ['literature', 'research', 'study', 'journal', 'evidence', 'published']
        findings_text = ' '.join(findings).lower()
    ...
    def _has_logical_chain(self, findings: List[str]) -> bool:
        ...
        logic_indicators = ['therefore', 'because', 'due to', 'result of', 'caused by', 'leading to']
        findings_text = ' '.join(findings).lower()
    ...
    def _has_physician_credentials(self, findings: List[str]) -> bool:
        ...
        credential_indicators = ['md', 'm.d.', 'do', 'd.o.', 'physician', 'doctor', 'licensed']
        findings_text = ' '.join(findings).lower()
    ...
    def _has_proper_medical_terminology(self, findings: List[str]) -> bool:
        ...
        medical_terms = ['diagnosis', 'condition', 'symptom', 'treatment', 'prognosis', 'etiology']
        findings_text = ' '.join(findings).lower()
        return sum(1 for term in medical_terms if term in findings_text) >= 2
    
    def _generate_component_rationale(self, component: str, criteria_scores: Dict[str, int], 
                                    findings: List[str], issues: List[str]) -> str:
    ...
    def _create_error_score_result(self, error_message: str) -> Dict:
        ...
        return {
            ...
            'service_connection_breakdown': ScoreBreakdown(0, 25, {}, "Error in scoring"),
            'medical_rationale_breakdown': ScoreBreakdown(0, 25, {}, "Error in scoring"),
            'professional_format_breakdown': ScoreBreakdown(0, 25, {}, "Error in scoring"),
