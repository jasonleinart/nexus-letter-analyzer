    ...
    try:
        ...
        mock_analysis = {
            'medical_opinion': {
                'score': 22,
                'confidence': 90,
                'findings': ['Clear probability language present', 'Definitive medical opinion'],
                'issues': [],
            ...
            'service_connection': {
                'score': 20,
                'confidence': 85,
                'findings': ['Explicit service connection stated', 'Specific service events mentioned'],
                'issues': [],
            ...
            'medical_rationale': {
                'score': 18,
                'confidence': 80,
                'findings': ['Medical literature referenced', 'Clinical explanation provided'],
        ...
        scoring_success = (
            ...
            0 <= scoring_results['overall_score'] <= 100 and
            'medical_opinion_breakdown' in scoring_results
        )
        
        results['scoring_engine'] = scoring_success
        if scoring_success:
            print(f"   ✓ Scoring engine working: {scoring_results['overall_score']}/100")
        else:
        ...
        rec_success = (
            'workflow_recommendation' in recommendations and
            'improvement_suggestions' in recommendations and
            'client_summary' in recommendations
        )
        
        results['recommendation_engine'] = rec_success
        if rec_success:
            workflow_rec = recommendations['workflow_recommendation']
            decision = workflow_rec.decision if hasattr(workflow_rec, 'decision') else 'unknown'
        ...
        analytics_success = (
            'total_analyses' in metrics and
            'avg_score' in metrics and
            'net_savings' in roi_metrics
        )
        
        results['analytics_engine'] = analytics_success
        if analytics_success:
            print(f"   ✓ Analytics working: {metrics['total_analyses']} analyses tracked")
        else:
